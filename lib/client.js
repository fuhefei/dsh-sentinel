	...
	factory: (require) => {
		...
		* The tab mounts via an eager check plus the service-landing event instead. */
		...
		function apply(ctx) {
			...
			const sidebarNow = ctx.betterSidebar;
			if (sidebarNow !== void 0) mountSidebarTab(sidebarNow);
			else ctx.on("internal/service", ((name) => {
				if (name !== "betterSidebar") return;
				const sidebar = ctx.betterSidebar;
				if (sidebar !== void 0) mountSidebarTab(sidebar);
			}));
		}
		//#endregion
