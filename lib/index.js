...
function renderWakeup(sub, fact) {
	return [
		...
		`你注册时留给自己的便签:\n${sub.note}`,
		"",
		fact.fireNumber >= sub.maxFires ? "这是最后一次触发，该订阅已自动结束。" : `剩余 ${String(sub.maxFires - fact.fireNumber)} 次触发额度；如果不再需要，用 sentinel 工具 cancel("${sub.id}")。`
...
var SentinelRuntime = class {
	...
	*/
	...
	async create(sessionId, spec, note, maxFires, cooldownSeconds, expiresInSeconds) {
		...
		const watch = this.watchOf(sessionId);
		if (watch.folded.active.size >= 16) throw new SentinelLogError(`subscription limit reached (${String(16)}); cancel one first`);
...
function registerSentinelTools(runtime, toolCtx, agent) {
	...
	disposers.push(toolCtx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => textBlock(value.hookPath !== void 0 ? `哨兵订阅 ${value.id} 已注册: webhook · ${value.target}\n推送地址: POST ${value.hookPath}（需常驻 dsh 进程监听该地址；一次性 headless 进程退出后无法接收推送）` : `哨兵订阅 ${value.id} 已注册: ${value.kind} · ${value.target}（每 ${String(value.intervalSeconds)}s 探测${value.kind === "file" ? "，文件变化即时推送" : ""}；会话休眠时由服务端值守；探测与触发投递由常驻 dsh 进程承担——一次性 headless 进程退出后，需有常驻进程（如 dsh web）运行才会生效）`)
		},
		async execute(args) {
			...
			const cooldown = args.cooldown_seconds !== void 0 ? Math.max(0, Math.round(args.cooldown_seconds)) : 60;
			const expires = args.expires_in_seconds !== void 0 ? Math.max(HEARTBEAT_MS / 1e3, Math.round(args.expires_in_seconds)) : void 0;
			...
			return {
				...
				...subscription.spec.kind === "webhook" ? { hookPath: `${HOOK_PATH}?id=${subscription.id}` } : {}
			};
		}
	})));
	disposers.push(toolCtx.tools.register(defineTool({
...
function watchRowHtml(row) {
	...
	const lastState = row.lastState !== void 0 ? escapeHtml(row.lastState) : "探测中";
	const next = row.kind === "webhook" ? "即时推送" : row.nextDueAt !== void 0 ? `${String(Math.max(0, Math.ceil((row.nextDueAt - Date.now()) / 1e3)))}s` : "…";
...
<tbody id="rows">${rows.length === 0 ? "<tr><td colspan=\"8\" class=\"empty\">当前没有活跃的监控。</td></tr>" : rows.map(watchRowHtml).join("")}</tbody>
</table>
<script>
...
//#endregion
export { DASHBOARD_PATH, HOOK_PATH, STATE_PATH, apply, dashboardHtml, inject, name, storePath };
